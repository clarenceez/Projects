...
namespace QIC.Farm.Service
{
    public class FarmService
    {
        ...
        {
            FamilyInfoDal dal = new FamilyInfoDal();
            //先通过手机号 查询familyID,如果openid为空，则是第一次绑定，familyInfo表加入openid
            var familyInfo = dal.GetFamilyInfoByPhoneNumber(phoneNumber);
            if (string.IsNullOrEmpty(familyInfo.OpenID))
            {
                //familyInfo加入openid nickname
                dal.SetOpenIDByPhone(phoneNumber, openID, nickname);
            }           

            //familyWechat表 插入数据
            FamilyWechatDal familyWechatDal = new FamilyWechatDal();
            familyWechatDal.Add(new FamilyWechatDto { FamilyID = familyInfo.ID, OpenID = openID });
            return dal.SetOpenIDByPhone(phoneNumber, openID,nickname);
        }
        ...
        {
            FamilyWechatDal dal = new FamilyWechatDal();
            return dal.IsOpenIDExist(openID);
        }
        ...
        {
            ProductReportDal dal = new ProductReportDal();
            return dal.GetProductReportByID(id);
        }
        ...
        //查询某个家庭的配送单(包括配送单的详情)
        public CommonSearchResult<DistributionDto> GetDistributionListByOpenID(string openID,int pageIndex,int pageSize)
        {
            DistributionDal dal = new DistributionDal();
            return dal.GetDistributionListByOpenID(openID,pageIndex,pageSize);
        }
        ...
        public DistributionDetailResult AddDistributionDetail(DistributionDetailDto distributionDetail)
        {
            ...
            if (!familyCardDal.IsTotalCountEnough(distributionDetail.FamilyCardID))
            {
                result.DistributionDetailError = DistributionDetailError.CountNotEnough;
                return result;
            }
            ...
            var success = dal.Add(distributionDetail);
            if (success)
            {
                result.DistributionDetailError = DistributionDetailError.Success;
            }
            return result;
        }
        ...
        public DistributionDetailResult UpdateDistributionDetail(DistributionDetailDto distributionDetail)
        {
            ...
            var success = dal.Update(distributionDetail);
            if (success)
            {
                result.DistributionDetailError = DistributionDetailError.Success;
            }
